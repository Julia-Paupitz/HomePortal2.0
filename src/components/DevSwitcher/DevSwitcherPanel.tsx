...
import { useDevSwitcher, type GlobalState, type LayoutMode, type NotificationFlags, type LoanNavMode, type ColorTheme } from '@/context/DevSwitcherContext'
...
const colorThemes: { key: ColorTheme; label: string; description: string }[] = [
  { key: 'default',  label: 'Default',         description: 'Dark navy header, gray background' },
  { key: 'teal-bg',  label: 'Teal Background',  description: 'Light teal sidebar, teal page, dark right panel' },
  { key: 'teal-nav', label: 'Teal Navigation',   description: 'Teal sidebar, white background' },
]

const loanNavModes: { key: LoanNavMode; label: string }[] = [
  { key: 'single',   label: 'Single Loan' },
  { key: 'option-a', label: 'Option A — List page' },
  { key: 'option-b', label: 'Option B — Dropdown' },
]

export function DevSwitcherPanel() {
  const { isOpen, setIsOpen, layoutMode, setLayoutMode, globalState, setGlobalState, componentVariations, setComponentVariation, notifications, setNotification, loanNavMode, setLoanNavMode, colorTheme, setColorTheme, reset } = useDevSwitcher()
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetContent side="right" className="w-[300px] p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          {/* Theme */}
          <div>
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Theme</div>
              ...
              {colorThemes.map(theme => (
                ...
                >
                  <span className={cn(
                    ...
                    colorTheme === theme.key ? 'border-teal-700 bg-teal-700' : 'border-gray-300 bg-white'
                  )} />
                  <span className="min-w-0">
                    <span className={cn('text-sm block', colorTheme === theme.key ? 'font-semibold text-navy-800' : 'text-gray-600')}>
                      {theme.label}
                    </span>
                    <span className="text-[11px] text-gray-400">{theme.description}</span>
                  </span>
                </button>
              ))}
            </div>
          ...
          {/* Loan Navigation (active-loan only) */}
          {globalState === 'loan-funded' && (
            <div>
              ...
              <div className="space-y-2">
                {loanNavModes.map(mode => (
                  <button
                    key={mode.key}
                    onClick={() => setLoanNavMode(mode.key)}
                    className={cn(
                      'w-full flex items-center gap-3 text-left px-3 py-2.5 rounded-lg border transition-colors',
                      loanNavMode === mode.key
                        ? 'border-navy-800/30 bg-navy-800/5'
                        : 'border-gray-200 hover:border-gray-300 bg-white'
                    )}
                  >
                    <span className={cn(
                      'w-3 h-3 rounded-full border-2 shrink-0',
                      loanNavMode === mode.key ? 'border-teal-700 bg-teal-700' : 'border-gray-300 bg-white'
                    )} />
                    <span className={cn('text-sm', loanNavMode === mode.key ? 'font-semibold text-navy-800' : 'text-gray-600')}>
                      {mode.label}
                    </span>
                  </button>
                ))}
